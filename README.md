...
Author: Vian Lelo
Date created: March 20th, 2026, Last Updated: March 25th, 2026
...
Notably, Segatella copri (formerly Prevotella copri) was visually prominent in vegan samples in Figure 1 but did not appear among the top 20 differentially abundant taxa in Figure 4. This discrepancy likely reflects inconsistent abundance across the three vegan samples: ANCOM‑BC2 is powered to detect consistent directional differences within groups, so a taxon that is high in one vegan sample but absent or low in the others will not rank highly by p‑value. This illustrates a key limitation of small‑n compositional analysis, where visually striking patterns in individual samples do not necessarily correspond to robust group‑level biological signals.

The absence of statistically significant differential abundance results (FDR < 0.05) is most plausibly explained by insufficient statistical power at n = 3 per group rather than a true absence of biological differences. The PERMANOVA R² of 0.191 indicates that diet explains approximately 19% of community‑level variation in beta diversity, a biologically meaningful effect size that is underpowered at this sample size. The directional consistency of fold changes with published literature—fibre‑fermenting taxa enriched in vegans and bile‑tolerant or protein‑fermenting taxa enriched in omnivores—further supports the biological relevance of these trends and aligns with known metabolic consequences of plant‑ versus animal‑based diets (David et al., 2014; Sonnenburg & Bäckhed, 2016). Future studies with larger sample sizes (for example, ≥20 individuals per group) and better control of confounders will be required to achieve adequate power to formally confirm these findings. Interactive taxonomic composition across all six samples is provided as a supplementary HTML file (krona_output/all_samples_krona.html), offering a hierarchical overview of community structure across taxonomic ranks.

The Krona visualization also revealed that approximately 99% of reads remained unclassified after Kraken2, likely reflecting the reduced coverage of the memory‑capped Standard‑8 database relative to the full Standard database, residual host DNA contamination, and the use of a conservative confidence threshold of 0.15 to reduce false positives. Because Bracken re‑estimates abundances using only the classified fraction, relative comparisons between samples remain internally valid, but a substantial portion of the true microbial community is not represented in diversity and differential abundance analyses. Future work should therefore use a more comprehensive reference database and explicit host read decontamination prior to classification to improve sensitivity.


## **Limitations**
The primary limitations of this analysis are the small sample size (n = 3 per group) and the very low Kraken2 classification rate (0.64–1.33% across samples). The small sample size restricts statistical power for both beta diversity and differential abundance testing—PERMANOVA was limited to 719 permutations—and no taxa reached FDR < 0.05 despite directionally consistent trends in line with prior literature. The low classification rate likely reflects the reduced reference coverage of the Standard‑8 database, residual host DNA, and the conservative confidence threshold, implying that much of the community remains uncharacterized. In addition, individual‑level confounders such as age, geography, antibiotic history, and BMI were not controlled for and may contribute to within‑group variability. Future studies should incorporate the full Standard Kraken2 database, aggressive host read removal, and at least 10–20 samples per group to achieve adequate power and better control for confounding.

Overall, these results support the hypothesis that long‑term vegan and omnivore diets are associated with distinct gut microbiome compositions, particularly in short‑chain‑fatty‑acid‑producing taxa and bile‑tolerant species, even though the small sample size limits formal statistical significance.
